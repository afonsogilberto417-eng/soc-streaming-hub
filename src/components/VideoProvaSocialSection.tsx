...
const VideoProvaSocialSection = () => {
  ...
  const pendingPlayRef = useRef<boolean[]>([false, false, false]);

  const getStartTime = (index: number) => (index === 2 ? 1 : 0);

  const pauseOtherPlayers = useCallback((currentIndex: number) => {
    playerRefs.current.forEach((player, idx) => {
      if (idx !== currentIndex && player) {
        player.pause().catch(() => undefined);
      }
    });
  }, []);

  const resetToStartAndPause = useCallback((index: number) => {
    const player = playerRefs.current[index];
    if (!player) return;

    const startTime = getStartTime(index);
    player.setCurrentTime(startTime).catch(() => undefined);
    player.pause().catch(() => undefined);
  }, []);

  const initPlayer = useCallback((index: number, iframe: HTMLIFrameElement) => {
    ...
    player.on("play", () => {
      player.setVolume(0.5).catch(() => undefined);
      pauseOtherPlayers(index);
    });

    player.on("timeupdate", ({ seconds, duration }) => {
      if (duration > 0 && seconds >= duration - 1) {
        resetToStartAndPause(index);
      }
    });
    ...
    if (pendingPlayRef.current[index]) {
      pendingPlayRef.current[index] = false;
      player.setVolume(0.5).catch(() => undefined);
      player.play().catch(() => undefined);
    }
  }, [pauseOtherPlayers, resetToStartAndPause]);
  ...
  const handleThumbnailClick = (index: number) => {
    pendingPlayRef.current[index] = true;
    pauseOtherPlayers(index);
    ...
    const existingPlayer = playerRefs.current[index];
    if (existingPlayer) {
      pendingPlayRef.current[index] = false;
      existingPlayer.setVolume(0.5).catch(() => undefined);
      existingPlayer.play().catch(() => undefined);
    }
  };

  const getIframeSrc = (videoId: string, index: number) => {
    const startTime = index === 2 ? "1s" : "0s";
    return `https://player.vimeo.com/video/${videoId}?autoplay=0&muted=0&loop=0&title=0&byline=0&portrait=0&badge=0&dnt=1&controls=1&transparent=0&quality_selector=0&fullscreen=0&settings=0&pip=0&airplay=0&cc=0&outro=0` + `#t=${startTime}`;
  };
                ...
                {activePlayers[index] ? (
                  <>
                    <iframe
                      ...
                      onLoad={(e) => initPlayer(index, e.currentTarget)}
                    />
                    {/* Block top bar */}
                    <div
